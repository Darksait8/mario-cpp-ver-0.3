...
template <>
constexpr inline auto GetX(const box2d::Body& body)
{
    return body.GetLocation().x;
}
...
template <>
constexpr inline auto GetY(const box2d::Body& body)
{
    return body.GetLocation().y;
}
...
// Явная перегрузка для box2d::Body
inline Angle GetAngle(const box2d::Body& body)
{
...
// Явная перегрузка для box2d::Body
inline auto GetLengthSquared(const box2d::Body& body) noexcept
{
    const auto loc = body.GetLocation();
    return loc.x * loc.x + loc.y * loc.y;
}

inline auto GetLength(const box2d::Body& body)
{
    const auto loc = body.GetLocation();
    return std::sqrt(loc.x * loc.x + loc.y * loc.y);
}
